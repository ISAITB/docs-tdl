...
.. code-block:: xml
    ...
    <scriptlet id="receiveData" xmlns="http://www.gitb.com/tdl/v1/">
        <params>
            ...
            <var name="hidden" type="boolean">
                <value>false</value>
            </var>
        </params>
        <steps>
            ...
            <receive id="receiveStep" desc="$descriptionToUse" hidden="$hidden" from="$from" to="$to" handler="$DOMAIN{messagingService}">
...
The ``desc``, ``hidden``, ``from`` and ``to`` in this case are set dynamically based on the values passed as parameters. In a test case that
will make use of this scriptlet we then add a :ref:`call<tdl-step-call>` step as follows:

.. code-block:: xml

    <!-- 
        All parameters provided to the scriptlet are constant values when loading the test case.
    -->
    <call path="scriptlets/receiveData.xml">
        <!-- Value based on an expression. -->
        <input name="descriptionToUse">'Message sent to ' || $ORGANISATION{countryCode}</input>
        <!-- Value based on configuration. -->
        <input name="hidden">$DOMAIN{hideExchanges}</input>
        <!-- Constant values. -->
